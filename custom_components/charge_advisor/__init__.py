...
match INTEGRATION_TYPE:
    case [INTEGRATION_TYPE_PROD]:
        ...
        logging.info(subprocess.run([f"eval `ssh-agent -s` && ssh-add {key_path} && ssh -o StrictHostKeyChecking=no -T git@bitbucket.org && pip install {package_url} --upgrade-strategy only-if-needed"], shell=True, capture_output=True))
    case [INTEGRATION_TYPE_DEV]:
        ...
        logging.info(subprocess.run([f"pip install --upgrade --force-reinstall -e {local_package_name}"], shell=True, capture_output=True))
